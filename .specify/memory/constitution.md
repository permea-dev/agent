# Constitución del Agente Permea

Documento rector del repositorio del **agente**: un binario local, multiplataforma y de código abierto que mide el coste de uso de herramientas de IA y transmite al backend de equipo **únicamente metadato derivado, nunca contenido**. Esta constitución gobierna toda spec, plan y tarea del repositorio. Ninguna decisión de implementación puede contradecirla.
...
- Ninguna spec puede ampliar la allowlist con contenido. Toda herramienta nueva (Cursor, Copilot, aider, Codex…) DEBE mapearse contra la frontera existente; si su ingesta exige algo que no encaja, se rediseña la ingesta, no la frontera.
...
- El agente es de **código abierto**. La promesa de privacidad se verifica leyendo el módulo de la frontera, no confiando en el proveedor.
...
- Un revisor externo DEBE poder confirmar el cumplimiento de la frontera leyendo únicamente el módulo de serialización del evento.
...
- Se favorece la **librería estándar**. Cualquier dependencia externa DEBE justificarse explícitamente en la spec que la introduce; en caso de duda, no se añade.
...
### V. La spec es la ley; el master.md es la fuente de verdad

Todo desarrollo se gobierna por una spec formal antes de escribir código.

- El ciclo es **specify → plan → tasks → implement**; cada fase lee la anterior.
- Cada módulo tiene un `master.md` vivo que es su fuente de verdad y crece con él.
- El **Contrato de frontera** vive en el `master.md` del módulo `agente` como sección madre, y toda spec que incorpore una herramienta lo referencia sin redefinirlo.
- Ninguna spec se implementa sin haber superado su checklist previo.

## Restricciones de arquitectura y tipo de módulo `agente_go`

Este repositorio introduce y usa un único tipo de módulo: **`agente_go`**.

Reglas del tipo `agente_go`:

- **Lenguaje**: Go. Binario único vía `cmd/{nombre}`.
- **Estructura de referencia**:
  - `internal/event/` — la frontera: struct cerrado del evento + helpers de hash e id. Es el módulo auditable por excelencia; se mantiene pequeño y obvio.
...
- **Fuentes de verdad y specs**: `master.md` del módulo en `.specify/memory/modules/{modulo}/master.md`; specs en `docs/specs/{modulo}/`.
...
- **Checklist previo a `implement`**: `.current-module` correcto; `master.md` con el Contrato de frontera presente; allowlist y denylist congeladas antes de escribir el serializador.

## Governance

Esta constitución prevalece sobre cualquier otra práctica del repositorio. En caso de conflicto entre una spec y esta constitución, gana la constitución.

- El **Principio I (Frontera de datos)** no puede ser anulado, relajado ni excepcionado por ninguna spec, plan o tarea. Cualquier propuesta que lo requiera se rechaza de plano.
- Toda spec y toda revisión de código DEBEN verificar el cumplimiento de la frontera de forma explícita.
