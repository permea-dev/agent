...
- [ ] T007 [US1] Añadir el despacho del subcomando en `cmd/permea/main.go`: si `os.Args[1] == "enroll"` → `runEnroll(os.Args[2:])` y salir con código según error; **conservar** intactos los flags `--scan/--run/--daemon/--version`.
- [ ] T008 [US1] Test de camino feliz en `cmd/permea/enroll_test.go` con `httptest.NewTLSServer` (200) y `HOME`/config dir temporal: `runEnroll` con el string por **argv** persiste `endpoint`+`device_token`; `config.json` a **0600**; la salida incluye la **URL** y **no** el token (SC-001, SC-002, SC-003).
- [ ] T009 [US1] Test de la vía **stdin** y SC-011 en `cmd/permea/enroll_test.go`: alimentar el string por stdin (`-` y sin argumento) produce el mismo estado que por argv; aseverar que el token **no** aparece en `os.Args` durante la llamada (SC-011) y que **no** se hace eco a la salida (FR-015). Añadir la aserción del **camino de error** (edge case A1): `runEnroll` sin argumento y con stdin **no-pipe** (TTY simulada / sin input) → **error de uso, exit ≠ 0**, **no** se cuelga y **no** persiste `config.json` (FR-001).
