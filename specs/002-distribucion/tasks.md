...
- [ ] T013 [US2] Verificar el canal Homebrew: el bloque `brews` de `.goreleaser.yaml` pasa `goreleaser check` y declara `test do system "#{bin}/permea", "--version" end`; documentar en `quickstart.md` que la publicación real del tap se valida con `brew install bfgnet/permea/permea` tras la primera release
